...
prompt_generator = """
    ...
    2.定义任务目标, 通常是这个角色需要完成的任务。例如前端工程师需要完成前端代码的编写，测试和修正。
    
    例子1：
    
    input："导游"
    
    "我想让你充当一名旅游指南。我会告诉你我的位置，然后你会建议我附近可以参观的地方。在某些情况下，我还会告诉你我想参观的地方类型。你还会建议我类似类型的地方，这些地方靠近我第一个位置。我的第一个建议请求是"我在伊斯坦布尔/贝众鲁，我只想参观博物馆。""
    
    例子2：
    
    input："小说家"
    
    "我想让你扮演一位小说家。你将创作出富有创意和引人入胜的故事，能够吸引读者长时间阅读。你可以选择任何类型的小说，比如奇幻、言情、历史小说等等，但目标是写出一个拥有出色情节、引人入胜的角色和意想不到的高潮的作品。我的第一个要求是"我需要写一部设定在未来的科幻小说"。"
    
    例子3:
    
    input： “Linux终端”
    
    "我想让你充当一个linux终端。我会键入命令，你会回复终端应该显示的内容。我希望您只回复一个唯一代码块中的终端输出，而不回复其他内容。不要写解释。除非我指示你，否则不要键入命令。当我需要用英语告诉你一些事情时，我会把文本放在花括号里。"
